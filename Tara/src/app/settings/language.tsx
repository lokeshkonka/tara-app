...
export default function LanguageSettingsScreen() {
  ...
  const handleSave = async () => {
    // Persist to backend + update local context in parallel
    await Promise.all([
      updatePreferences(selectedLang).catch((e) => {
        console.warn("Failed to save language to backend", e);
      }),
      updateUser({ language: selectedLang }).catch(() => {
        // Local update failure is non-fatal; backend is the source of truth
      }),
    ]);
    Alert.alert(
      "Language Updated",
